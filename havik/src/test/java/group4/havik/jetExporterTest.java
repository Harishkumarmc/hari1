...
import group4.havik.jetExporter;
 ...
 * {@link jetExporter} class. Feel free to adapt and enhance this test class
 ...
 * @author sri
 */
public class jetExporterTest extends PepperExporterTest {
	/**
	...
	public void test_DummyImplementation() {
		...
		getFixture().setCorpusDesc(new CorpusDesc().setCorpusPath(URI.createFileURI(getTempPath("jetExporter").getAbsolutePath())));

		// starts the Pepper framework and the conversion process
		start();

		File superCorpus = new File(getTempPath("jetExporter").getAbsolutePath() + "/rootCorpus");
